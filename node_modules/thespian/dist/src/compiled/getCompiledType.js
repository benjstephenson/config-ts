...
const getCompiledType = (type, elementaryClassSet, enumMap) => {
    ...
    switch (type.kind) {
        ...
        case typescript_1.SyntaxKind.FunctionType:
            return new TType_1.TArrow(mapParameters(typeAny.parameters, elementaryClassSet, enumMap), exports.getCompiledType(typeAny.type, elementaryClassSet, enumMap));
        ...
        case typescript_1.SyntaxKind.ArrayType:
            return new TType_1.TArray(exports.getCompiledType(typeAny.elementType, elementaryClassSet, enumMap));
...
const handleTypeReference = (type, elementaryClassSet, enumMap) => {
    const name = getCompiled_1.getName(type.typeName);
    ...
    return new TType_1.TClass(name, exports.mapGenericArguments(type.typeArguments, elementaryClassSet, enumMap));
};
const mapElements = (elements, elementaryClassSet, enumMap) => elements.filter(e => ofType_1.ofType.isObject(e)).map(e => exports.getCompiledType(e, elementaryClassSet, enumMap));
const mapParameters = (parameters, elementaryClassSet, enumMap) => parameters.map(p => new TType_1.TParam(p.name.escapedText, exports.getCompiledType(p.type, elementaryClassSet, enumMap)));
...
const mapGenericParameters = (typeParameters) => (typeParameters || []).filter(e => ofType_1.ofType.isObject(e)).map(t => new TType_1.TGenericParameter(getCompiled_1.getName(t.name)));
