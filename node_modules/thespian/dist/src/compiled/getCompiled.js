...
const getCompiledClass = (node, decompiledArray, elementaryClassSet, enumMap) => {
    const name = exports.getName(node.name);
    const genericParameters = getCompiledType_1.mapGenericParameters(node.typeParameters);
...
const getCompiledInterface = (node, elementaryClassSet, enumMap) => {
    ...
    node.members.forEach(member => {
        if (member.kind === typescript_1.SyntaxKind.MethodSignature) {
            methods.push({
                name: exports.getName(member.name),
                genericParameters: getCompiledType_1.mapGenericParameters(member.typeParameters),
                parameters: getParameters(member.parameters, elementaryClassSet, enumMap),
                resultType: getCompiledType_1.getCompiledType(member.type, elementaryClassSet, enumMap)
            });
        }
...
const getParameterType = (param, elementaryClassSet, enumMap) => {
    ...
    return getCompiledType_1.getCompiledType(param.type, elementaryClassSet, enumMap);
};
