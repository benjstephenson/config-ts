...
const generateValidator = (fileName, elementaryClassSet, enumMap) => {
    ...
    const results = [];
    const classDetails = getCompiled_1.getCompiled(sourceFile, elementaryClassSet, enumMap);
    classDetails.forEach(classDetail => exports.makeBuilder(classDetail, results));
    if (results.length > 0) {
    ...
    switch (compiled.type) {
        ...
        case Compiled_1.CompiledType.InterfaceType:
            ...
            compiled.fields.forEach(field => {
                makeBuilder = true;
                fieldValidators.push(fillTemplate_1.fillTemplate(fieldValidatorTemplate, {
                    fieldName: field.name,
                    matcher: field.resultType.matcher()
                }));
            });
            if (makeBuilder) {
                results.push(fillTemplate_1.fillTemplate(validatorTemplate, {
