...
describe("array.length:", () => {
    describe("assertThat():", () => {
        it('matches', () => {
            const actual = ["b", "b"];
            assertThat_1.assertThat(actual).is(match_1.match.array.length(2));
        });
        it('does not match', () => {
            const actual = ["a", "b"];
            assertThat_1.assertThat(actual).failsWith(match_1.match.array.length(1), { [MatchResult_1.MatchResult.was]: 2, [MatchResult_1.MatchResult.expected]: { "array.length": 1 } });
        });
    ...
    describe("validateThat():", () => {
        ...
        it("succeeds", () => {
            const validation = validateThat_1.validateThat(["b", "b"]).satisfies(expected);
            assertThat_1.assertThat(validation.passed()).is(true);
        });
        it("fails", () => {
            const validation = validateThat_1.validateThat(["b", "b", "b"]).satisfies(expected);
