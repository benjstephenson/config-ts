...
describe("DescribeMatcher()", () => {
    ...
    describe("assertThat():", () => {
        it("matches", () => {
            assertThat_1.assertThat(4).is(expected);
        });
        it("mismatches", () => {
            assertThat_1.assertThat(2).failsWith(expected, { [MatchResult_1.MatchResult.was]: 2, [MatchResult_1.MatchResult.expected]: 4 });
        });
        it("mismatches: errors", () => {
            const mismatched = [];
            expected.mismatches(new DiffMatcher_1.ContextOfValidationError(), mismatched, 2);
            assertThat_1.assertThat(mismatched).is(["actual: four"]);
        });
    });
    describe("validateThat():", () => {
        it("succeeds", () => {
            const validation = validateThat_1.validateThat(4).satisfies(expected);
        ...
        it("fails with specified context", () => {
            ...
            const validation = validateThat_1.validateThat({ id: 1, f: 3 }).satisfies(expected);
            assertThat_1.assertThat(validation.passed()).is(false);
            assertThat_1.assertThat(validation.errors).is([
        ...
        it("Provides tailored validation error messages", () => {
            ...
            const validation = validateThat_1.validateThat({ personId: 11, name: 3, age: -1 }).satisfies(expected);
            assertThat_1.assertThat(validation.passed()).is(false);
            assertThat_1.assertThat(validation.errors).is([
    ...
    it("describe example", () => {
        const results = validateThat_1.validateThat({ f: "a" })
            .satisfies({
            f: match_1.match.describe(match_1.match.ofType.number(), actual => `four, not ${actual}`)
        });
        assertThat_1.assertThat(results.errors).is(["four, not a"]);
    });
});
