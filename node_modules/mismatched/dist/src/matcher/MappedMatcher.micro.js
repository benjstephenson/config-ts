...
describe("MappedMatcher()", () => {
    describe("assertThat():", () => {
        ...
        it("matches", () => {
            assertThat_1.assertThat({ m: 2 }).is(matcher);
        });
        it("mismatches", () => {
            assertThat_1.assertThat(3).failsWith(matcher, {
        ...
        it("Handles a string mapped", () => {
            ...
            assertThat_1.assertThat(actual).is({ detail: match_1.match.mapped(JSON.parse, { f: [0] }, 'json') });
        });
    });
    describe("validateThat():", () => {
        const matcher = match_1.match.mapped(a => a.m, match_1.match.ofType.number(), { extract: "m" });
        it("succeeds", () => {
            const validation = validateThat_1.validateThat({ m: 2 }).satisfies(matcher);
            assertThat_1.assertThat(validation.passed()).is(true);
        });
        it("fails", () => {
            const validation = validateThat_1.validateThat({ m: false }).satisfies(matcher);
