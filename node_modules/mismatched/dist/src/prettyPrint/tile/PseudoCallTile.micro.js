...
describe("PseudoCallTile", () => {
    it("Undefined arguments", () => {
        ...
        assertThat_1.assertThat(arrayTile).is({ callName: "target.meth", stringLength: 3, complexity: 1, withSquareBrackets: false });
        assertThat_1.assertThat(Appender_1.Appender.composeTile(arrayTile)).is("target.meth");
    });
    it("No arguments", () => {
        const arrayTile = new PseudoCallTile_1.PseudoCallTile("target.meth", []);
        assertThat_1.assertThat(arrayTile).is({ callName: "target.meth", args: [], stringLength: 11, complexity: 3, withSquareBrackets: false });
        assertThat_1.assertThat(Appender_1.Appender.composeTile(arrayTile)).is("target.meth()");
    });
    it("One argument", () => {
        const tile = new SimpleTile_1.SimpleTile([1, 2, 3]);
        const arrayTile = new PseudoCallTile_1.PseudoCallTile("fn", [tile]);
        assertThat_1.assertThat(arrayTile).is({ callName: "fn", args: [tile], stringLength: 9, complexity: 4, withSquareBrackets: false });
        assertThat_1.assertThat(Appender_1.Appender.composeTile(arrayTile)).is('fn(1,2,3)');
    });
    it("Three arguments", () => {
        ...
        assertThat_1.assertThat(arrayTile).is({
        ...
        assertThat_1.assertThat(Appender_1.Appender.composeTile(arrayTile)).is("dateTime.plusDays(1,2,3, 4, true)");
    });
