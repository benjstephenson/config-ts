...
class Assertion {
    ...
    isNot(expected) {
        this.checkForFunction();
        return this.is(match_1.match.not(matchMaker_1.matchMaker(expected)));
    }
    isAnyOf(expected) {
        this.checkForFunction();
...
expected: '${printer.render(message)}'`);
    ...
    throws(expected = match_1.match.any()) {
        ...
        const matcher = matchMaker_1.matchMaker(expected);
        let passed = false;
        try {
            this.actual();
            passed = true;
            ...
            if (!result.passed()) {
                result.bad(PrettyPrinter_1.exceptionMessage(e));
            }
        }
        if (passed) {
...
class ExceptionAssertion {
    ...
    catchWithMessage(expected) {
        ...
        if (failed) {
            assert_1.fail(`Failed to throw exception. Expecting ${expected} but got ${result}`);
        }
    }
    catch(expected) {
        let failed = false;
